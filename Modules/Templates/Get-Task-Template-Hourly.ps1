<#
.SYNOPSIS
Get-Task-Template-Hourly.ps1 This template creates an Daily task that repeats Hourly.
...
CLEANUP:
--------
Use a cleanup module "Get-Whatever_z_Cleanup_Tasks_All.ps1" to remove all
the scheduled tasks that you specify in that module.  (LOG-MD has a sample task cleanup)
...
$TaskName = "My Kewl Task"
# Description of the scheduled task
$TaskDescr = "Create a Hourly Check for Something checks Task"
...
$TaskArg = "Whatever arguments you have -o $ARTHIR_OutputDir"
...
If ($WriteEventLogEntry -eq 'No') {
  ...
  elseif ([System.Diagnostics.EventLog]::SourceExists($EventSource) -eq $False) {
    ...
    Write-EventLog -LogName Application -EntryType Information -EventId $Event_ID -Source $EventSource -Message 'Added Task AutoRuns VT Hourly by Arthir'
 }
  else {
    Write-EventLog -LogName Application -EntryType Information -EventId $Event_ID -Source $EventSource -Message 'Added Task AutoRuns VT Hourly by Arthir'
    }