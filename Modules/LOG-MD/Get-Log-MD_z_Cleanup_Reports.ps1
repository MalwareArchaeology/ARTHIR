<#
.SYNOPSIS
Get-LOG-MD_z_Cleanup_Reports.ps1 is designed to remove all items and reports 
...
of the $DeleteReports variable to remove all files.
...
Adjust the variables to what you want to do with each item:
  ...
  $DeleteReports				Yes/No - Deletes all the reports after the module has completed
...
#
If ($DeleteReports -eq 'No') {
  Write-Output "Reports not being deleted"
  }  
  else {
    Remove-Item -path $ARTHIR_OutputDir -recurse -force
}
#
#  Write log entry
#
If ($WriteEventLogEntry -eq 'No') {
  ...
  elseif ([System.Diagnostics.EventLog]::SourceExists($EventSource) -eq $False) {
    ...
    Write-EventLog -LogName Application -EntryType Information -EventId $Event_ID -Source $EventSource -Message 'LOG-MD Cleanup completed deleting all reports executed by Arthir'
 }
  else {
    Write-EventLog -LogName Application -EntryType Information -EventId $Event_ID -Source $EventSource -Message 'LOG-MD Cleanup completed deleting all reports executed by Arthir'
    }
