<#
.SYNOPSIS
Get-LOG-MD_Reg_Baseline.ps1 creates a Reg_Baseline.txt file that can be used on other 
systems to exclude known good registry keys and values.

NOTE:  This should ONLY be used on a system you know is "CLEAN".  The Reg_Baseline.txt
...
DOWNLOAD C:\Program Files\LMD\Results\*Reg_Baseline*
#>
...
$ARTHIR_ReportName = "Reg_Baseline.txt"
$Keep_Baseline = "Yes"
$RenameReports = "Yes"
...
$TaskName = "LOG-MD-Reg-Baseline-Test"
# Description of the scheduled task
$TaskDescr = "Run a LOG-MD Reg Baseline Task"
# Name of Tool used
...
$Event_ID = "1337"
#
#  Delete existing reports Reg_Baseline and $SysName-Reg_Baseline*
#
  Remove-Item -path $ARTHIR_OutputDir\*Reg_Baseline* -Force | Out-Null
  Remove-Item -Path $ARTHIR_Dir\Reg_Keep_Baseline.txt -Force | out-file -Append -filepath $ARTHIR_OutputDir\$SysName-Reg_Baseline_Status.txt
  Write-Output "Creating a temporary scheduled task for LOG-MD to run the Reg_Baseline feature" | out-file -Append -filepath $ARTHIR_OutputDir\$SysName-Reg_Baseline_Status.txt
#
...
if ($tasks | select Name | ? { $_.Name -eq $TaskName }) {
    ...
    Write-Output " $TaskName - Task does not already exist on the system" | out-file -Append -filepath $ARTHIR_OutputDir\$SysName-Reg_Baseline_Status.txt
}
#
#  Create Reg Baseline Task to run LOG-MD
#
...
#
#  Run LOG-MD Reg Baseline Task
#
if (Test-Path $ARTHIR_Dir\$Tool_Name) {
    Write-Output "Starting Scheduled Task to run Reg Baseline" | out-file -Append -filepath $ARTHIR_OutputDir\$SysName-Reg_Baseline_Status.txt
    SchTasks.exe /Run /TN $TaskName | out-file -Append -filepath $ARTHIR_OutputDir\$SysName-Reg_Baseline_Status.txt
} else {
...
#
Write-Output "Waiting for Reg_Baseline to be created..." | out-file -Append -filepath $ARTHIR_OutputDir\$SysName-Reg_Baseline_Status.txt
...
if (Test-Path $ARTHIR_Dir\$ARTHIR_ReportName) {
    Write-Output "LOG-MD Created $ARTHIR_ReportName" | out-file -Append -filepath $ARTHIR_OutputDir\$SysName-Reg_Baseline_Status.txt
} else {
...
If ($RenameReports -eq 'No') {
  Write-Output "Reports not being renamed" | out-file -Append -filepath $ARTHIR_OutputDir\$SysName-Reg_Baseline_Status.txt
  }  
  else {
    Write-Output "Renaming and listing reports" | out-file -Append -filepath $ARTHIR_OutputDir\$SysName-Reg_Baseline_Status.txt
    Copy-Item $ARTHIR_Dir\$ARTHIR_ReportName -Destination $ARTHIR_OutputDir\$ARTHIR_ReportName -Force
    Start-Sleep -s 60
    Rename-Item -Path $ARTHIR_OutputDir\$ARTHIR_ReportName -NewName $ARTHIR_OutputDir\$SysName-$ARTHIR_ReportName | out-file -Append -filepath $ARTHIR_OutputDir\$SysName-Reg_Baseline_Status.txt
   }
#
#  Keep Reg_Baseline if wanted
#
If ($Keep_Baseline -eq 'No') {
  Remove-Item -Path $ARTHIR_Dir\$ARTHIR_ReportName -Force | out-file -Append -filepath $ARTHIR_OutputDir\$SysName-Reg_Baseline_Status.txt
  Break
  } else {
    Rename-Item -Path $ARTHIR_Dir\$ARTHIR_ReportName -NewName Reg_Keep_Baseline.txt | out-file -Append -filepath $ARTHIR_OutputDir\$SysName-Reg_Baseline_Status.txt
  }
#
#  Remove LOG-MD-Reg-Baseline Task
#
    Write-Output "Deleting completed task" | out-file -Append -filepath $ARTHIR_OutputDir\$SysName-Reg_Baseline_Status.txt
    SchTasks.exe /Delete /TN \$TaskName /F | out-file -Append -filepath $ARTHIR_OutputDir\$SysName-Reg_Baseline_Status.txt
#
#  Write log entry
#
If ($WriteEventLogEntry -eq 'No') {
  Write-Output "Skipping event log entry" | out-file -Append -filepath $ARTHIR_OutputDir\$SysName-Reg_Baseline_Status.txt
  Break
  }  
  elseif ([System.Diagnostics.EventLog]::SourceExists($EventSource) -eq $False) {
    New-EventLog -LogName Application -Source $EventSource
    Write-Output "Event log entry created" | out-file -Append -filepath $ARTHIR_OutputDir\$SysName-Reg_Baseline_Status.txt
    Write-EventLog -LogName Application -EntryType Information -EventId $Event_ID -Source $EventSource -Message 'LOG-MD Reg-Baseline executed by Arthir'
 }
  else {
    Write-Output "Event log entry created" | out-file -Append -filepath $ARTHIR_OutputDir\$SysName-Reg_Baseline_Status.txt
    Write-EventLog -LogName Application -EntryType Information -EventId $Event_ID -Source $EventSource -Message 'LOG-MD Reg-Baseline executed by Arthir'
    }
  Write-Output "DONE - Reg Baseline creation completed" | out-file -Append -filepath $ARTHIR_OutputDir\$SysName-Reg_Baseline_Status.txt
  Get-ChildItem -Path "$ARTHIR_Dir\*Reg_*.txt" | out-file -Append -filepath $ARTHIR_OutputDir\$SysName-Reg_Baseline_Status.txt
  Get-ChildItem -Path "$ARTHIR_OutputDir\*Reg_*.txt" | out-file -Append -filepath $ARTHIR_OutputDir\$SysName-Reg_Baseline_Status.txt
# END