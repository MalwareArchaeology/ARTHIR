<#
.SYNOPSIS
Get-LOG-MD_Hash_Compare_Folder.ps1 creates a Hash_Compare.txt of a folder
 - e.g. C:\Users
...
DOWNLOAD C:\Program Files\LMD\Results\*Hash_Compare_*
#>
...
$TaskArg = "-hc -hd $ARTHIR_Baseline_Dir"
...
#
if ((test-path $ARTHIR_Dir\Hash_Baseline.txt) -or (test-path $ARTHIR_Dir\Hash_Keep_Baseline_Folder.txt)) {
#if (Test-Path $ARTHIR_Dir\Hash_Baseline.txt) {
    Write-Output "Hash_Baseline exists, continuing..." | out-file -Append $ARTHIR_OutputDir\$SysName-Hash_Compare_Status.txt
    Rename-Item -Path $ARTHIR_Dir\Hash_Keep_Baseline_Folder.txt -NewName $ARTHIR_Dir\Hash_Baseline.txt | out-file -Append -filepath $ARTHIR_OutputDir\$SysName-Hash_Compare_Status.txt
    } else {
...
if (Test-Path $ARTHIR_Baseline_Dir) {
    Write-Output "$ARTHIR_Baseline_Dir exists, baseline can continue" | out-file -Append -filepath $ARTHIR_OutputDir\$SysName-Hash_Compare_Status.txt
 } else {
...
#  Delete existing reports Hash_Baseline and $SysName-Hash_Baseline*
#
  Remove-Item -Path $ARTHIR_Dir\Hash_Keep_Baseline_Folder.txt -Force | out-file -Append -filepath $ARTHIR_OutputDir\$SysName-Hash_Compare_Status.txt
  Write-Output "Creating a temporary scheduled task for LOG-MD to run the Hash_Baseline feature" | out-file -Append -filepath $ARTHIR_OutputDir\$SysName-Hash_Compare_Status.txt
#
...
If ($RenameReports -eq 'No') {
  Write-Output "Reports not being renamed" | out-file -Append -filepath $ARTHIR_OutputDir\$SysName-Hash_Compare_Status.txt
    ...
    Start-Sleep -s 60
    Move-Item -Path $ARTHIR_Dir\Hash_Compare*.txt -Destination $ARTHIR_OutputDir
    Move-Item -Path $ARTHIR_Dir\Hash_Locked_Files* -Destination $ARTHIR_OutputDir
    Rename-Item -Path $ARTHIR_OutputDir\Hash_Locked_Files.csv -NewName $ARTHIR_OutputDir\$SysName-Hash_Compare_Locked_Files.csv | out-file -Append -filepath $ARTHIR_OutputDir\$SysName-Hash_Compare_Status.txt
    Rename-Item -Path $ARTHIR_OutputDir\$ARTHIR_ReportName -NewName $ARTHIR_OutputDir\$SysName-Hash_Compare_Folder.txt | out-file -Append -filepath $ARTHIR_OutputDir\$SysName-Hash_Compare_Status.txt
   }
#
#  Keep Hash_Baseline if wanted
#
If ($Keep_Baseline -eq 'No') {
    Write-Output " Not keeping $ARTHIR_Dir\$ARTHIR_ReportName_baseline" | out-file -Append -filepath $ARTHIR_OutputDir\$SysName-Hash_Compare_Status.txt
  } else {
    Copy-Item $ARTHIR_Dir\Hash_Baseline.txt -Destination $ARTHIR_Dir\Hash_Keep_Baseline_Folder.txt | out-file -Append -filepath $ARTHIR_OutputDir\$SysName-Hash_Compare_Status.txt
    Remove-Item -Path $ARTHIR_Dir\Hash_Baseline.txt -Force | out-file -Append -filepath $ARTHIR_OutputDir\$SysName-Hash_Compare_Status.txt
    Remove-Item -Path $ARTHIR_Dir\Hash_Latest.txt -Force | out-file -Append -filepath $ARTHIR_OutputDir\$SysName-Hash_Compare_Status.txt
  }
#
