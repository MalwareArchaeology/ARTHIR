<#
.SYNOPSIS
Get-LOG-MD_Running_Processes.ps1 returns output from all running processes
and their modules to find malicious or unknown running processes.
...
DOWNLOAD C:\Program Files\LMD\Results\*Report_Process_and_Modules_Running*
#>
$Tool_Name = "LOG-MD.exe"
$ARTHIR_Dir = "C:\Program Files\LMD"
$ARTHIR_OutputDir = "C:\Program Files\LMD\Results"
$ARTHIR_ReportName = "Report_Process_and_Modules_Running.csv"
$RenameReports = "Yes"
...
if (Test-Path $ARTHIR_Dir\$Tool_Name) {
    ...
    & $ARTHIR_Dir\$Tool_Name -proc -o $ARTHIR_OutputDir 
} else {
...
If ($RenameReports -eq 'No') {
  Write-Output "Reports not being renamed"
	...
	Move-Item -Path Report_Process_and_Modules*.csv -Destination $ARTHIR_OutputDir
    Get-ChildItem $ARTHIR_OutputDir\Report_Process_and_Modules* | Rename-Item -NewName { $_.name -Replace 'Report_Process_and_Modules',"$SysName-Report_Process_and_Modules" } 
}
