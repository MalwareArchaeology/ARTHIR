<#
.SYNOPSIS
Get-LOG-MD_Logs_6_Days.ps1 harvests 6 days of log events that LOG-MD
determines as security relevant.
...
This script does depend on IMF Security's LOG-MD-Pro.exe, which is not
...
copy the LOG-MD-Pro.exe binary to each remote target's ADMIN$ share (C:\Windows)
...
If ($RenameReports -eq 'No') {
  Write-Output "Reports not being renamed"
    ...
    Move-Item -Path Report* -Destination $ARTHIR_OutputDir
    Get-ChildItem $ARTHIR_OutputDir\Report_* | Rename-Item -NewName { $_.name -Replace 'Report_',"$SysName-Report_" } 
    Rename-Item $ARTHIR_OutputDir\_Report_List.txt -NewName $SysName-Report_List.txt
    Rename-Item $ARTHIR_OutputDir\Report.csv -NewName $SysName-Report.csv
}
#
#  Write log entry
#
