<#
.SYNOPSIS
Get-LOG-MD_Hash_Baseline.ps1 creates a Hash_Baseline.txt of the 
entire C: Drive that can be used on other systems to exclude known good file hashes during a Hash Compare. 
...
#  Delete existing reports Hash_Baseline and $SysName-Hash_Baseline*
#
  Remove-Item -Path $ARTHIR_Dir\Hash_Keep_Baseline.txt -Force | out-file -filepath $ARTHIR_OutputDir\$SysName-Hash_Baseline_Status.txt
...
If ($RenameReports -eq 'No') {
  Write-Output "Reports not being renamed" | out-file -Append -filepath $ARTHIR_OutputDir\$SysName-Hash_Baseline_Status.txt
    ...
    Start-Sleep -s 30
    Copy-Item '$ARTHIR_Dir\Hash_Locked_Files.csv' -Destination '$ARTHIR_OutputDir\Hash_Locked_Files.csv' -Force
    ...
    Rename-Item -Path '$ARTHIR_OutputDir\$ARTHIR_ReportName' -NewName '$ARTHIR_OutputDir\$SysName-$ARTHIR_ReportName' | out-file -Append -filepath $ARTHIR_OutputDir\$SysName-Hash_Baseline_Status.txt
   }
#
...
If ($Keep_Baseline -eq 'No') {
    ...
    Copy-Item "$ARTHIR_OutputDir\$SysName-Hash_Baseline.txt" -Destination "$ARTHIR_Dir\$ARTHIR_ReportName" | out-file -Append -filepath $ARTHIR_OutputDir\$SysName-Hash_Baseline_Status.txt
    Rename-Item -Path $ARTHIR_Dir\$ARTHIR_ReportName -NewName Hash_Keep_Baseline.txt | out-file -Append -filepath $ARTHIR_OutputDir\$SysName-Hash_Baseline_Status.txt
  }
#
