<#
.SYNOPSIS
Get-Net-IP-Interface.txt.ps1 - This will list the Network IP interfaces.
 - Looks for IP interfaces
...
DOWNLOAD C:\Program Files\ARTHIR\*Report_Net-IP-Interface.txt
#>
$ARTHIR_OutputDir = "C:\Program Files\ARTHIR"
$ARTHIR_ReportName = "Report_Net-IP-Interface.txt"
...
#  List the ARP table
#
Write-Output "List Network IP Interfaces on - $SysName" | Out-File -FilePath $ARTHIR_OutputDir\$SysName-$ARTHIR_ReportName
...
if  (Get-Command Get-NetIPInterface -ErrorAction SilentlyContinue) {
    Get-NetIPInterface -AddressFamily IPv4 | Out-File -Append -FilePath $ARTHIR_OutputDir\$SysName-$ARTHIR_ReportName
    Write-Output "---------------------------------------------------------------------------" | Out-File -Append -FilePath $ARTHIR_OutputDir\$SysName-$ARTHIR_ReportName
    Get-NetIPInterface -AddressFamily IPv6 | Out-File -Append -FilePath $ARTHIR_OutputDir\$SysName-$ARTHIR_ReportName
} else {
(Get-WmiObject -Class Win32_NetworkAdapterConfiguration -Filter MACAddress!=null |
  Select-Object -Property DNSHostName, Index, IPAddress, IPEnabled, Description, Caption, DHCPEnabled, DNSServerSearchOrder, DefaultGateway, MACAddress -ExcludeProperty IPX*,WINS*) |
    Out-File -Append -FilePath $ARTHIR_OutputDir\$SysName-$ARTHIR_ReportName
 }
#
#  Write log entry
#
