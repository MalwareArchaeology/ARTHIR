<#
.SYNOPSIS
Get-Local_Admin_Accounts.ps1 - This will collect all local administrator accounts.
 - Looks for 
...
DOWNLOAD C:\Program Files\ARTHIR\*Report_Local_Admin_Accounts.txt
#>
$ARTHIR_OutputDir = "C:\Program Files\ARTHIR"
$ARTHIR_ReportName = "Report_Local_Admin_Accounts.txt"
...
#
Write-Output "List of Local Administrator accounts on - $SysName" | Out-File -FilePath $ARTHIR_OutputDir\$SysName-$ARTHIR_ReportName
Write-Output "-------------------------------------------------------" | Out-File -Append -FilePath $ARTHIR_OutputDir\$SysName-$ARTHIR_ReportName
invoke-command {
 net localgroup administrators |  where {$_ -AND $_ -notmatch "command completed successfully"} | 
 select -skip 4 | Out-File -Append -FilePath $ARTHIR_OutputDir\$SysName-$ARTHIR_ReportName
 }
#
#  Write log entry
#
