<#
.SYNOPSIS
Get-Anti-MW-HealthStatus.ps1 - This reports Windows Defender status (Win 10, WIn 2016 and later).
...
DOWNLOAD C:\Program Files\ARTHIR\*Report_Anti-MW-Health-Status.txt
#>
$ARTHIR_OutputDir = "C:\Program Files\ARTHIR"
$ARTHIR_ReportName = "Report_Anti-MW-Health-Status.txt"
...
#
#  Check Windows Anti-Malware Health Status
#
Write-Output "Windows Anti-Malware Health Status" | out-file -Filepath $ARTHIR_OutputDir\$SysName-$ARTHIR_ReportName 
Write-Output "-------------------------------------------------------------------------" | out-file -Append -Filepath $ARTHIR_OutputDir\$SysName-$ARTHIR_ReportName 
Get-WmiObject -namespace root\Microsoft\SecurityClient -Class AntimalwareHealthStatus | format-list | out-file -Append -Filepath $ARTHIR_OutputDir\$SysName-$ARTHIR_ReportName 
#
#  Write log entry
#
If ($WriteEventLogEntry -eq 'No') {
  ...
  elseif ([System.Diagnostics.EventLog]::SourceExists($EventSource) -eq $False) {
    ...
    Write-EventLog -LogName Application -EntryType Information -EventId $Event_ID -Source $EventSource -Message 'Windows Anti-Malware Health Status queried by Arthir'
 }
  else {
    Write-EventLog -LogName Application -EntryType Information -EventId $Event_ID -Source $EventSource -Message 'Windows Anti-Malware Health Status queried by Arthir'
    }
