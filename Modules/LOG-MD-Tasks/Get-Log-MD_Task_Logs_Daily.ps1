<#
.SYNOPSIS
Get-Log-MD_Logs_Daily_Task.ps1 creates a daily task to run LOG-MD -1 log harvest.
  - Runs LOG-MD -1 -o $ARTHIR_OutputDir
...
$TaskName = "Test_LOG-MD_1-Day_of_Logs"
# Description of the scheduled task
$TaskDescr = "Create a LOG-MD Daily Check for 1 Day of Logs"
...
#
# Create Schedule Task to run Daily 1 Day of Logs
#
...
If ($WriteEventLogEntry -eq 'No') {
  ...
  elseif ([System.Diagnostics.EventLog]::SourceExists($EventSource) -eq $False) {
    ...
    Write-EventLog -LogName Application -EntryType Information -EventId $Event_ID -Source $EventSource -Message 'Added Task Logs Daily by Arthir'
 }
  else {
    Write-EventLog -LogName Application -EntryType Information -EventId $Event_ID -Source $EventSource -Message 'Added Task Logs Daily by Arthir'
    }